...
from tkinter import StringVar, Frame, Widget, Tk, Label, Entry
from typing import Union, Any, List

from model import process_trigrams
from textwrap import wrap

from script.model import Trigrams


def main():
    def delete_excess_children_widgets():
        children: list[Widget] = root.winfo_children()
        if len(children) >= 4:
            no_match_label.grid_forget()

    def manage_excess_label():
        text: str = word_gen_label_value.get()
        text = "\n".join(wrap(text, 45))
        word_gen_label_value.set(text)

    def format_likely_words(tail_pairs: list) -> str:
        total_freq: Union[int, Any] = sum([val for _, val in tail_pairs])
    ...
    def my_tracer(*args):
        ...
        split_text: list[str] = new_text.split()
        word_gen_label_value.set(new_text)
        delete_excess_children_widgets()
        if len(split_text) >= 2 and new_text[-1] != " ":
            tails: Union[str, tuple] = trigram_model.get_n_tail(split_text[-2:], False)
            if tails != "not found":
    ...
    # -------------- EXECUTION --------------
    trigram_model: Trigrams = process_trigrams()
    root: Tk = tk.Tk()
    set_window_config()
    main_frame: Frame = tk.Frame(root, width=350, height=340)
    main_frame.grid(row=0, column=0, rowspan=2)
    main_frame.propagate(0)

    # creation of linked variable, from entry to word_gen_label
    entry_value: StringVar = tk.StringVar()
    entry_value.trace("w", my_tracer)

    word_gen_label_value: StringVar = tk.StringVar()

    # creation of both word_gen_label and entry with shared variable
    word_gen_label: Label = tk.Label(main_frame, textvariable=word_gen_label_value)  # .grid(row=0, column=0, padx=30
    word_gen_label.pack(pady=30, padx=30)
    word_gen_label.propagate(True)
    entry: Entry = tk.Entry(main_frame, textvariable=entry_value, width=45)  # .grid(row=1, column=0, pady=30, padx=30)
    entry.pack(pady=30, padx=30)
    no_match_label: Label = tk.Label(root, text="")

    # most likely words label
    lwl_frame: Frame = tk.Frame(root, width=200, height=340)
    lwl_frame.propagate(0)
    lwl_frame.grid(row=0, column=1, rowspan=2)
    likely_words_label_value: StringVar = tk.StringVar()
    likely_words_label: Label = tk.Label(lwl_frame, textvariable=likely_words_label_value, justify="left", padx=40)
